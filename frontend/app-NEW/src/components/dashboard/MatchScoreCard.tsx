...
import { getMatchColor, getMatchBgColor } from '@/data/mockData';
...
export function MatchScoreCard({ matchScore }: MatchScoreCardProps) {
  ...
  useEffect(() => {
    springScore.set(matchScore.score);
  ...
  const scoreColor = getMatchColor(matchScore.score);
  const scoreBgColor = getMatchBgColor(matchScore.score);
