...
export function RecruitmentHero({ notice }: RecruitmentHeroProps) {
  const [copied, setCopied] = useState(false);

  const getStatusBadgeStyle = (color: UniversalRecruitmentNotice['statusBadgeColor']) => {
    switch (color) {
      case 'emerald':
  ...
  const shareText = `🚨 *${notice.title}*\n🏢 Authority: ${notice.authority}\n${notice.vacancy?.total ? `📊 Vacancies: ${notice.vacancy.total}\n` : ''}${notice.dates.applicationLastDate ? `⏳ Last Date: ${notice.dates.applicationLastDate}\n` : ''}Check eligibility, syllabus & apply online: ${shareUrl}`;
    ...
    <section className="relative pt-6 pb-6 overflow-hidden border-b border-white/5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <span className="px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-zinc-300 text-xs font-semibold uppercase tracking-wider">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-6">
            ...
            {notice.vacancy?.total && (
              <div className="flex items-center gap-1.5 text-emerald-400 font-bold">
                ...
                <span>{notice.vacancy.total} Posts</span>
              </div>
            )}
