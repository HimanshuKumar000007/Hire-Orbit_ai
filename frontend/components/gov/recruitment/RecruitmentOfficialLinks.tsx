...
import { Link2, ExternalLink, ShieldCheck, ArrowRight, Zap, FileText } from 'lucide-react';
import Link from 'next/link';

interface RecruitmentOfficialLinksProps {
  links: UniversalNoticeLink[];
  authority: string;
}

export function RecruitmentOfficialLinks({ links, authority }: RecruitmentOfficialLinksProps) {
      ...
      <div className="overflow-x-auto rounded-xl border border-white/5">
        <table className="w-full text-left text-xs sm:text-sm">
          <thead className="bg-white/5 text-zinc-400 font-semibold border-b border-white/5 text-xs">
            <tr>
              <th className="py-3 px-4">Official Resource / Action</th>
