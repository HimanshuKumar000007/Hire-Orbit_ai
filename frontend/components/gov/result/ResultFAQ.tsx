...
interface ResultFAQProps {
  faqs: Array<{ question: string; answer: string }>;
  examName: string;
}

export function ResultFAQ({ faqs, examName }: ResultFAQProps) {
      ...
      <div className="flex items-center gap-2.5 mb-5 pb-3 border-b border-white/5">
        ...
        <div>
          <h2 className="text-base font-bold text-white tracking-tight">
            Frequently Asked Questions ({examName} Result)
          </h2>
          <p className="text-xs text-zinc-400">
            Verified candidate inquiries regarding result declaration, scorecards, and cutoff marks
          </p>
        </div>
      </div>
