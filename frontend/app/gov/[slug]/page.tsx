  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100 selection:bg-emerald-500/30 pt-16 lg:pt-20">
      ...
      <article className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* ── TYPE DETECTION FOR DYNAMIC PRESENTATION ─────────────────────── */}
        {(() => {
          ...
          return (
            <>
              {/* 🌟 SARKARI RESULT MASTER 3-COLUMN COMMAND CENTER (The Standard of Indian Aspirants) */}
              <section className="py-8">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-emerald-400">
                    <Sparkles className="w-3.5 h-3.5" /> Official Gazette Overview &amp; Key Specifications
                  </div>
                  <span className="text-[11px] font-semibold text-zinc-400 px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10">
                    {enriched.domainName || "Government Service"}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="rounded-2xl border border-white/10 bg-zinc-900/90 p-5 space-y-3">
                    <div className="flex items-center gap-2 pb-2.5 border-b border-white/5">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-white tracking-wide">Important Dates</h3>
                        <p className="text-[10px] text-zinc-400">Official schedule &amp; deadlines</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 text-xs divide-y divide-white/5">
                      ...
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">Admit Card Release:</span>
                        <span className="font-semibold text-emerald-400">{job.importantDates.admitCardDate || (isExamNotice ? "Available Now / Upcoming" : "Before Examination")}</span>
                      </div>
                      {job.importantDates.resultDate && (
                        <div className="flex justify-between items-center pt-2">
                          <span className="text-zinc-400">Result Declaration:</span>
                          <span className="font-semibold text-purple-400">{job.importantDates.resultDate}</span>
                        </div>
                      )}
                    </div>
                  </div>

                  {/* BOX 2: APPLICATION FEE DETAILS */}
                  <div className="rounded-2xl border border-white/10 bg-zinc-900/90 p-5 space-y-3">
                    <div className="flex items-center gap-2 pb-2.5 border-b border-white/5">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-white tracking-wide">Application Fee</h3>
                        <p className="text-[10px] text-zinc-400">Category-wise prescribed charges</p>
                      </div>
                    </div>

                    <div className="space-y-2 text-xs divide-y divide-white/5">
                      <div className="flex justify-between items-center pt-1.5">
                        <span className="text-zinc-400">General / OBC / EWS:</span>
                        <span className="font-bold text-white">{job.applicationFee.generalOBC || "See Notification"}</span>
                      </div>
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">SC / ST / PH (PwBD):</span>
                        <span className="font-bold text-emerald-400">{job.applicationFee.scStPh || "Exempted / Nil"}</span>
                      </div>
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">All Female Candidates:</span>
                        <span className="font-semibold text-purple-300">{job.applicationFee.female || "Exempted / As per rules"}</span>
                      </div>
                      <div className="pt-2">
                        <span className="text-zinc-500 text-[11px] block mb-1">Payment Gateway Options:</span>
                        <span className="text-[11px] text-zinc-300 font-medium bg-white/5 px-2.5 py-1 rounded-lg block border border-white/5">
                          Net Banking, Debit / Credit Card, UPI, or State E-Challan
                        </span>
                      </div>
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">Correction Charge:</span>
                  ...
                  <div className="rounded-2xl border border-white/10 bg-zinc-900/90 p-5 space-y-3">
                    <div className="flex items-center gap-2 pb-2.5 border-b border-white/5">
                      ...
                      <div>
                        <h3 className="text-sm font-bold text-white tracking-wide">Age Limit &amp; Eligibility</h3>
                        <p className="text-[10px] text-zinc-400">Minimum requirements as on cutoff date</p>
                      </div>
                    </div>

                    <div className="space-y-2 text-xs divide-y divide-white/5">
                      <div className="flex justify-between items-center pt-1.5">
                        <span className="text-zinc-400">Prescribed Age Limit:</span>
                        <span className="font-bold text-white">{job.ageLimit}</span>
                      </div>
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">SC / ST Age Relaxation:</span>
                        <span className="font-semibold text-emerald-400">+5 Years (Standard)</span>
                      </div>
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">OBC (Non-Creamy):</span>
                        <span className="font-semibold text-blue-400">+3 Years</span>
                      </div>
                      <div className="flex justify-between items-center pt-2">
                        <span className="text-zinc-400">PwBD Candidates:</span>
                        <span className="font-semibold text-purple-400">+10 to 15 Years</span>
                      </div>
                      <div className="pt-2">
                        <span className="text-zinc-500 text-[11px] block mb-1">Key Qualification:</span>
                        <span className="text-[11px] text-emerald-300 font-medium bg-emerald-500/10 px-2.5 py-1 rounded-lg block border border-emerald-500/20 line-clamp-2" title={job.qualification}>
                          {job.qualification}
                        </span>
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                  <div className="p-3 rounded-xl bg-zinc-900/60 border border-white/5">
                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold block">Total Vacancies</span>
                    <span className="text-sm font-bold text-white">{job.vacancies}</span>
                  </div>
                  <div className="p-3 rounded-xl bg-zinc-900/60 border border-white/5">
                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold block">Pay Scale</span>
                    <span className="text-sm font-bold text-emerald-400 truncate block" title={job.payScale}>{job.payScale}</span>
                  </div>
                  <div className="p-3 rounded-xl bg-zinc-900/60 border border-white/5">
                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold block">Job Cadre</span>
                    <span className="text-sm font-bold text-zinc-200 truncate block">{enriched.domainName || "State / Central"}</span>
                  </div>
                  <div className="p-3 rounded-xl bg-zinc-900/60 border border-white/5">
                    <span className="text-[10px] uppercase tracking-wider text-zinc-500 font-semibold block">Location</span>
                    <span className="text-sm font-bold text-white truncate block">{job.location}</span>
                  </div>
                </div>
              </section>

              {/* 🌟 CONTEXT-AWARE DYNAMIC ACTION COMMAND BAR */}
              <div className="flex flex-wrap items-center gap-3 p-4 rounded-2xl bg-white/[0.02] border border-white/5 mb-10">
                {isExamNotice ? (
                  <>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 sm:flex-initial px-6 py-3.5 rounded-xl bg-blue-500 hover:bg-blue-400 text-zinc-950 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-glow-sm"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 sm:flex-initial px-6 py-3.5 rounded-xl bg-white/10 hover:bg-white/15 text-white font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all border border-white/10"
                    >
                      <ExternalLink className="w-4 h-4" />
                      Official Exam Authority Portal
                    </a>
                  </>
                ) : isResultNotice ? (
                  <>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 sm:flex-initial px-6 py-3.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-glow-sm"
                    >
                  ...
                  <>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 sm:flex-initial px-6 py-3.5 rounded-xl bg-purple-500 hover:bg-purple-400 text-zinc-950 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-glow-sm"
                    >
                  ...
                  <>
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 sm:flex-initial px-6 py-3.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all shadow-glow-sm"
                    >
                ...
                <Link
                  href="/onboarding"
                  className="w-full sm:w-auto px-6 py-3.5 rounded-xl bg-white/5 hover:bg-white/10 text-emerald-400 font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all border border-emerald-500/20"
                >
              ...
              {/* 🌟 EXAM DAY DOCUMENTS & HALL INSTRUCTIONS (ONLY FOR EXAM DATES & ADMIT CARDS) */}
              {isExamNotice && (
                <section className="mb-12 rounded-3xl p-6 sm:p-8 bg-gradient-to-br from-blue-950/30 via-zinc-950/80 to-zinc-950 border border-blue-500/30 space-y-6">
                  <div className="flex items-center gap-2 text-blue-400 text-xs font-bold uppercase tracking-wider">
                    <ShieldCheck className="w-4 h-4" /> Candidate Exam Hall Advisory
                  </div>
                  <div>
                    <h3 className="text-xl sm:text-2xl font-bold text-white mb-1">
                      Mandatory Exam Day Checklist &amp; Hall Ticket Guidelines
                    </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                    ...
                    <div className="p-5 rounded-2xl bg-zinc-900/80 border border-white/5 space-y-3">
                      <h4 className="text-sm font-bold text-white flex items-center gap-2">
                        ...
                        Mandatory Documents to Carry to Exam Venue
                      </h4>
                      <ul className="space-y-2 text-xs text-zinc-300">
                        {(enriched.requiredDocuments || []).map((doc, dIdx) => (
                    ...
                    {/* Prohibited Items & Hall Rules */}
                    <div className="p-5 rounded-2xl bg-zinc-900/80 border border-white/5 space-y-3">
                      <h4 className="text-sm font-bold text-white flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4 text-amber-400" />
                        Exam Day Timing &amp; Strictly Prohibited Items
                      </h4>
                      <ul className="space-y-2 text-xs text-zinc-300">
                        {(enriched.examDayGuidelines || []).map((rule, rIdx) => (
        ...
        {/* 🌟 1. POST-WISE & DEPARTMENT-WISE VACANCY BREAKDOWN TABLE */}
        ...
        {/* 🌟 2. CATEGORY-WISE RESERVATION DISTRIBUTION (AUTHENTIC - NO FAKE DATA) */}
        <section className="mb-12">
          ...
          {enriched.isVacancyKnown ? (
            ...
            <div className="p-6 rounded-2xl bg-zinc-900/60 border border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="text-white font-bold text-sm flex items-center gap-2">
                  <ShieldCheck className="w-4 h-4 text-emerald-400" />
                  Official Gazette Reservation Quota Notice
                </div>
                <p className="text-xs text-zinc-400 max-w-2xl leading-relaxed">
                  Vertical reservation (UR, OBC, EWS, SC, ST) and horizontal reservation (Women, PwBD, Ex-Servicemen) are administered strictly in accordance with government roster guidelines in the official gazette circular. Consult the recruitment circular for cadre-wise allocation.
                </p>
              </div>
              <a
                href={job.officialPdfUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/15 text-white text-xs font-bold border border-white/10 shrink-0 inline-flex items-center gap-1.5 transition-colors"
              >
                <Download className="w-3.5 h-3.5" /> View Gazette Circular
              </a>
            </div>
          )}
        </section>
